...
function notcore(p) {
  // only include full path files (all others are core files which are huge)
  // maybe better solved by seeking into the file and reading exactly the part we need
...
function attachSourceToCallsites(callsites, includeSourceLines, cb) {
  ...
  const tasks = callsites
    .filter(function(x) { return x.allocations && x.allocations.length && notcore(x.script) })
